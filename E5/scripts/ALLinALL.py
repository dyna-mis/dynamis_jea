...
for mod in ['add', 'sub']:
    for model in ['uniform', 'gaussian']:
        ...
        outPutName = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'plots',  mod + '-' + model + '-' + 'update' + '-' + 'square')
        linePlot.getLine_Runtime(result_path, mod, model, 'rect', outPutName)
        print("recompute")
        outPutName = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'plots',  mod + '-' + model + '-' + 'recompute' + '-' + 'square')
