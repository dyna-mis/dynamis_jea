...
def getLine_Runtime(result_path, mod, model, skipName, outPutName):
    frames = []
    ...
    draw_Runtimes(result, outPutName)


def getLine_Runtime_recompute(result_path, mod, model, skipName, outPutName):
    frames = []
    for size in ['-1000-', '-2000-', '-4000-', '-8000-', '-16000-', '-32000-']:
        ...
        frames.append(df)
    result = pd.concat(frames)
    draw_Runtime_recompute(result, outPutName)





def export_legend(legend, filename="D:\GIT\Python\dynamis_jea_plots\E5\plots\legend.pdf"):
    fig  = legend.figure
    fig.canvas.draw()
...
def draw_Runtimes(result,name):
     ...
     #pq.set_xscale('log')
     plt.ylim([0.00008,1.2])
     leg = ax.legend(['line','g-line','grid','g-grid','grid-2','g-grid-2','grid-4','g-grid-4','MIS-ORS','MIS-graph'],loc='upper center', bbox_to_anchor=(0.5, -0.05),
