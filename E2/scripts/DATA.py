...
def algo_partition(path, instance, mod):
    dic = {
        "wmis":[],
        "line": [],
        "g-line": [],
    ...
    for x in path.iterdir():
        if x.is_file():
            if instance not in x.name:
                continue
            if mod in x.name:
                if "wmis" in x.name:
                    dic["wmis"].append(x.name)
                if "graph" in x.name:
    ...
    for key, value in dic.items():
        assert len(dic[key]) == 1, (key, dic[key])
...
class folderList:
    ...
    def print_info(self):
     for key,value in self.fL_data.items():
         #print key,
         value.print_info()
    def getExactMAXVALUE(self):
        # upate exact ratio
        for key,value in self.fL_data.items():
            if key == 'wmis':
                continue
            print(self.fL_data['wmis'])
            x = [float(b)/float(q) for b,q in zip(value.f_data['updateSize'],self.fL_data['wmis'].f_data["updateSize"])]
            value.f_data.update({'update sizeRatio':x})

    def __init__(self, result_path,instance, mod, skip):
        algo_dic = algo_partition(result_path, instance, mod)
        objs = list()
        ...
        self.getExactMAXVALUE()
        del self.fL_data['wmis']
